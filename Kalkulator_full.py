...
#untuk mengaktifkan mode gelap dan tema warna biru pada aplikasi
...
#font yang digunakan dalam aplikasi
...
#tabel AR untuk unsur-unsur kimia
AR = {
...
#Macam-macam indikator dan rentang pH nya
INDIKATOR_LIST = [
...
''' bagian def ini digunakan sebagai fungsi untuk frontend
kaya contohnya  make button untuk setting tombol gitu gitu'''
def to_float(s):
    try:
...
'''ini gabungan, bagian frontend nya tapi khusus bagian kalkulatornya, kaya misalakan bagian pengenceran
untuk di bawahnya kan ada def.... nah itu masuk ke backend di mana isinya logika seperti perhitungan untuk pengenceran'''
class KalkulatorPanel(ctk.CTkScrollableFrame):
    def __init__(self, master):
        super().__init__(master, fg_color=BG, scrollbar_button_color=BORDER, scrollbar_button_hover_color=ACCENT)
...
class TitrasiPanel(ctk.CTkFrame):
    def __init__(self, master):
        ...
        left = ctk.CTkScrollableFrame(self, fg_color=PANEL, corner_radius=12, scrollbar_button_color=BORDER)
        ...
        self.jenis.pack(fill="x", pady=(0, 6))

        self.jenis2 = ctk.CTkSegmentedButton(
            left,
            values=["Asam Poliprotik\n× Basa Kuat", "Basa Poliprotik\n× Asam Kuat"],
            font=("Courier New", 10, "bold"),
            fg_color=CARD, selected_color=ACCENT,
            selected_hover_color=ACCENT2,
            unselected_color=CARD, text_color=TEXT,
            command=self.on_jenis2_change)
        self.jenis2.pack(fill="x", pady=(0, 14))
        self.entries = {}
    ...
    def on_jenis2_change(self, val):
        self.jenis.set("")
        if "Asam Poliprotik" in val:
    ...
    def plot_titrasi(self):
        ...
        jenis2 = self.jenis2.get()
        try:
            if "Asam Poliprotik" in jenis2:
                V, pH = self.calc_poly_acid()
            elif "Basa Poliprotik" in jenis2:
